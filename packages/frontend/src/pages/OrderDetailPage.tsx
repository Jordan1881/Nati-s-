...
export default function OrderDetailPage() {
  ...
  if (isLoading) {
    return (
      <main dir="rtl" className="min-h-screen flex items-center justify-center text-gray-400 bg-[#FDFAF6]">
        טוען הזמנה...
      </main>
    )
  }
  ...
  if (isError || !order) {
    return (
      <main dir="rtl" className="min-h-screen flex flex-col items-center justify-center gap-4 text-gray-500 bg-[#FDFAF6]">
        <p>הזמנה לא נמצאה</p>
        <button
  ...
  const inputClass = "w-full border border-[#E8D8C4] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-brand-400 transition-colors"

  return (
    <main dir="rtl" className="min-h-screen bg-[#FDFAF6]">
      {/* Header */}
      <header className="bg-white/95 backdrop-blur-sm border-b border-[#F0E4D0] sticky top-0 z-10 px-4 py-3 flex items-center justify-between gap-4">
        <button
          onClick={() => (editMode ? exitEditMode() : navigate('/orders/today'))}
          className="flex items-center gap-1 text-brand-600 text-sm hover:text-brand-700 font-medium"
        >
          {editMode ? (
            <X size={16} />
          ) : (
        ...
        {editMode ? (
          <>
            ...
            <section className="bg-white rounded-2xl p-4 flex flex-col gap-3 border border-[#F0E4D0] shadow-md">
              ...
              <div>
                <label className="block text-sm font-medium mb-1 text-gray-700">סטטוס</label>
                <input
                  type="text"
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-brand-500 text-white font-bold text-sm hover:bg-brand-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-sm"
              >
            ...
            <section className="bg-white rounded-2xl p-4 flex flex-col gap-3 border border-[#F0E4D0] shadow-md">
                    ...
                    <div
                      ...
                      className={`rounded-xl p-3 flex flex-col gap-2 select-none transition-all ${
                        qty > 0
                          ...
                          : 'bg-white border border-[#F0E4D0] hover:border-brand-200'
                      }`}
                    >
                      ...
                      <div className="flex items-center justify-end gap-1 mt-auto">
                        {qty === 0 ? (
                          <button
                            ...
                            className="w-7 h-7 rounded-lg bg-brand-500 text-white text-base font-bold flex items-center justify-center hover:bg-brand-600 active:scale-95 transition-all"
                          >
                            +
                          </button>
                        ) : (
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className="w-7 h-7 rounded-lg bg-white border border-[#E8D8C4] hover:bg-gray-100 text-base font-bold flex items-center justify-center active:scale-95 transition-all"
                            >
                              −
                            </button>
                            <span className="w-5 text-center text-sm font-bold text-brand-700">{qty}</span>
                            <button
                              onClick={() => addToCart(item.id)}
                              className="w-7 h-7 rounded-lg bg-brand-500 text-white text-base font-bold flex items-center justify-center hover:bg-brand-600 active:scale-95 transition-all"
                            >
              ...
              {cartEntries.length > 0 && (
                <div className="border-t border-[#F0E4D0] pt-3 flex flex-col gap-1">
                  ...
                  <div className="flex justify-between font-black text-base border-t border-[#F0E4D0] pt-1 mt-1">
                    <span>סה״כ</span>
          ...
          <>
            ...
            <section className="bg-white rounded-2xl p-4 border border-[#F0E4D0] shadow-md">
              ...
              <div className="flex flex-col gap-1.5 text-sm">
                ...
                <div className="flex justify-between font-black text-base border-t border-[#F0E4D0] pt-2 mt-1">
                  <span>סה״כ</span>
            ...
            {/* ── Print actions ── */}
            <section className="bg-white rounded-2xl p-4 flex flex-col gap-2 border border-[#F0E4D0] shadow-md">
              <h2 className="font-bold mb-1">הדפסה</h2>
              <a
                href={`/print/order/${order.id}/customer`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl border-2 border-[#E8D8C4] text-gray-700 hover:bg-[#FDFAF6] text-sm font-medium transition-colors"
              >
                <Printer size={15} />
                הדפס שובר לקוח
              </a>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" dir="rtl">
          ...
          />
          <div className="relative bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl border border-[#F0E4D0]">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={deleteMutation.isPending}
                className="flex-1 py-2.5 rounded-xl border-2 border-[#E8D8C4] text-gray-700 hover:bg-[#FDFAF6] font-medium disabled:opacity-40 transition-colors"
              >
                ביטול
              </button>
            </div>
